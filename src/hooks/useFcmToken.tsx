...
const useFcmToken = () => {
  ...
  const [fcmToken, setFcmToken] = useState<string | null>(null); // State to store the FCM token.
  ...
  useEffect(() => {
    const setupListener = async () => {
      if (!fcmToken) return; // Exit if no token is available.
  ...
  return { fcmToken, notificationPermissionStatus }; // Return the token and permission status.
};
